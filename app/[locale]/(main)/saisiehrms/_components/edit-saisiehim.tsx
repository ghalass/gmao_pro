...
interface EditSaisiehimProps {
  saisiehim: Saisiehim & { panne: Panne; saisiehrm: Saisiehrm };
  open: boolean;
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      try {
        ...
        const response = await apiFetch(API.SAISIEHIMS.UPDATE(saisiehim.id), {
        ...
        if (response.ok) {
          router.refresh();
          toast.success("Saisie HIM mise à jour avec succès");
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Modifier l'intervention du{" "}
            <strong>
              {new Date(saisiehim.saisiehrm.du).toLocaleDateString()}
            </strong>
            .
          </DialogDescription>
        </DialogHeader>
