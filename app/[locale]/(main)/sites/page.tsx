...
import { SiteUpdateImport } from "./_components/SiteUpdateImport";
...
const SitesPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const sitesResponse = await apiFetch(`${API.SITES.ALL}?${params}`);
      ...
      const response: SitesResponse = sitesResponse.data;
  ...
  const handleExportFiltered = async () => {
    try {
      ...
      return sites.map((site) => ({
        ...
        "Engins associés": site._count?.engins || 0,
        "Date de création": new Date(site.createdAt).toLocaleDateString(
          "fr-FR"
        ),
        "Date de mise à jour": new Date(site.updatedAt).toLocaleDateString(
          "fr-FR"
        ),
      }));
    } catch (error) {
  ...
  const handleExportAll = async () => {
    try {
      ...
      const response = await apiFetch(`${API.SITES.ALL}?${params}`);
  ...
  const handleUpdateImportComplete = (result: any) => {
    ...
    if (result.success) {
      toast.success(
        `Modification réussie: ${result.summary?.updated || 0} sites mis à jour`
      );
      // Retour automatique à la liste après succès
      setTimeout(() => {
        setViewMode("list");
      ...
      <div className="mx-auto p-4">
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold mb-2">Modification de Sites</h1>
              <p className="text-sm text-muted-foreground">
                Modifiez des sites existants en masse depuis un fichier Excel
              </p>
            </div>
          </div>
        </div>
        ...
        <SiteUpdateImport onImportComplete={handleUpdateImportComplete} />
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <h1 className="text-xl sm:text-2xl font-bold">Sites</h1>
            {loading && (
          ...
          <p className="text-sm text-muted-foreground">
            {pagination?.totalItems || 0} site{plural} configuré{plural}
            {sites.length > 0 && searchTerm && (
              <span className="ml-2">
                ({sites.length} résultat{sites.length !== 1 ? "s" : ""} filtré
                {sites.length !== 1 ? "s" : ""})
              </span>
            )}
          </p>
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          <ExportExcel
            filename="sites_filtres"
            sheetName="Sites (filtrés)"
            onExportData={handleExportFiltered}
            className="flex items-center justify-center gap-2 w-full sm:w-auto"
          />

          {permissions.create && <NewSite onSuccess={refreshSites} />}
        </div>
      </div>
