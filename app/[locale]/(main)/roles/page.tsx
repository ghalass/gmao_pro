...
// app/[locale]/(main)/roles/page.tsx
...
import { RoleImport } from "./_components/RoleImport";
import { RoleUpdateImport } from "./_components/RoleUpdateImport";
...
const RolesPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const rolesResponse = await apiFetch(`${API.ROLES.ALL}?${params}`);
      ...
      const response: RolesResponse = rolesResponse.data;
  ...
  const handleExportFiltered = async () => {
    try {
      return roles.map((role) => ({
        ...
        Utilisateurs: role.user.length,
        "Date de création": new Date(role.createdAt).toLocaleDateString(
          "fr-FR"
        ),
        "Date de mise à jour": new Date(role.updatedAt).toLocaleDateString(
          "fr-FR"
        ),
      }));
    } catch (error) {
      ...
      <div className="mx-auto p-4">
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold mb-2">Modification de Rôles</h1>
              <p className="text-sm text-muted-foreground">
                Modifiez des rôles existants en masse depuis un fichier Excel
              </p>
            </div>
          </div>
        </div>

        <RoleUpdateImport onImportComplete={handleUpdateImportComplete} />
      </div>
    );
  }
    ...
    return (
      <div className="mx-auto p-4">
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold mb-2">Importation de Rôles</h1>
              <p className="text-sm text-muted-foreground">
        ...
        <RoleImport onImportComplete={handleImportComplete} />
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <h1 className="text-xl sm:text-2xl font-bold">Rôles</h1>
            {loading && (
          ...
          <p className="text-sm text-muted-foreground">
            {pagination?.totalItems || 0} rôle{plural} répertorié{plural}
            {roles.length > 0 && searchTerm && (
              <span className="ml-2">
                ({roles.length} résultat{roles.length !== 1 ? "s" : ""} filtré
                {roles.length !== 1 ? "s" : ""})
              </span>
            )}
          </p>
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          <ExportExcel
            filename="roles_filtres"
            sheetName="Rôles (filtrés)"
            onExportData={handleExportFiltered}
            className="flex items-center justify-center gap-2 w-full sm:w-auto"
          />
          <NewRole onSuccess={refreshRoles} />
        </div>
      </div>
              ...
              <TableRow>
                ...
                >
                  {searchTerm
                    ...
                    : "Aucun rôle répertorié"}
                </TableCell>
              </TableRow>
            ) : (
              <>
                {roles.map((role: RoleWithDetails) => (
                  <TableRow key={role.id}>
                    ...
                    <TableCell className="hidden sm:table-cell">
                      <Badge variant="outline">
                        {role.user.length} utilisateur
                        {role.user.length !== 1 ? "s" : ""}
                      </Badge>
                    </TableCell>
