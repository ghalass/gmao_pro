...
interface UserUpdateImportProps {
  onImportComplete: (result: any) => void;
}

export function UserUpdateImport({ onImportComplete }: UserUpdateImportProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/users/update-import", {
        method: "POST",
        body: formData,
      });
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      link.href = url;
      link.download = "users_update_template.xlsx";
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold">Modification d'utilisateurs</h3>
          <p className="text-sm text-muted-foreground">
            Modifiez des utilisateurs existants en masse depuis un fichier Excel
          </p>
        </div>
        <Button
          variant="outline"
      ...
      <Alert>
        ...
        <AlertDescription>
          ...
          <ul className="list-disc list-inside mt-2 space-y-1 text-sm">
            ...
            <li>
              L'email est obligatoire pour identifier l'utilisateur à modifier
            </li>
