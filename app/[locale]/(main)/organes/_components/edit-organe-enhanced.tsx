...
interface EditOrganeProps {
  organe: any & { type_organe: TypeOrgane };
  open: boolean;
...
const EditOrgane = ({ organe, open, onOpenChange }: EditOrganeProps) => {
  ...
  // Initialiser les filtres quand le dialogue s'ouvre
  useEffect(() => {
    if (open) {
      loadTypeParcs();
      // Trouver le parc et typeParc actuels de l'organe
      if (organe.type_organe) {
        // Charger les parcs pour trouver celui qui contient ce type d'organe
        apiFetch(API.PARCS.ALL).then((response) => {
          if (response.ok) {
            const allParcs = response.data || [];
            // Chercher un parc qui contient ce type d'organe dans ses associations
            const currentParc = allParcs.find((parc: any) =>
              parc.typeOrganes?.some(
                (to: any) => to.id === organe.type_organe.id
              )
            );
            if (currentParc) {
              setSelectedTypeParc(currentParc.typeparcId);
              setSelectedParc(currentParc.id);
              loadParcs(currentParc.typeparcId);
              loadTypeOrganes(currentParc.id);
            } else {
              // Fallback : chercher le premier parc et charger tous les types
              const firstParc = allParcs[0];
              if (firstParc) {
                setSelectedTypeParc(firstParc.typeparcId);
                setSelectedParc(firstParc.id);
                loadParcs(firstParc.typeparcId);
                loadTypeOrganes(firstParc.id);
              }
            }
          }
        });
            ...
            <div className="border rounded-lg p-4 bg-muted/30">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div className="space-y-2">
                  <FieldLabel>Type de parc</FieldLabel>
                  <Select
                ...
                <form.Field name="typeOrganeId">
                  {(field) => (
                    <div className="space-y-2">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {filteredTypeOrganes.map((typeOrgane) => (
                              <Badge
                                key={typeOrgane.id}
                                variant={
                                  field.state.value === typeOrgane.id
                                    ? "default"
                                    : "secondary"
                                }
                                className="cursor-pointer hover:bg-primary/80 transition-colors"
                                onClick={() =>
                                  field.handleChange(typeOrgane.id)
                                }
                              >
                                {typeOrgane.name}
                              </Badge>
                      ...
                      {filteredTypeOrganes.length === 0 && selectedParc && (
                        <div className="mt-3 p-3 bg-muted/30 rounded border">
                          <p className="text-sm text-muted-foreground">
                            Aucun type d'organe associé à ce parc. Utilisez
                            l'outil de gestion des associations pour lier des
                            types d'organes à ce parc.
                          </p>
                        </div>
                      )}
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-4">
                ...
                <form.Field name="active">
                  {(field) => (
                    <Field className="gap-0.5">
                      <FieldLabel htmlFor="active">Statut</FieldLabel>
                      <Select
