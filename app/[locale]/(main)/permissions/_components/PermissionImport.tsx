...
interface PermissionImportProps {
  onImportComplete: (result: any) => void;
}

export function PermissionImport({ onImportComplete }: PermissionImportProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/permissions/import", {
        method: "POST",
        body: formData,
      });
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      link.href = url;
      link.download = "permissions_template.xlsx";
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold">Importation de permissions</h3>
