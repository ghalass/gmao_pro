...
interface PermissionUpdateImportProps {
  onImportComplete: (result: any) => void;
}

export function PermissionUpdateImport({
  onImportComplete,
}: PermissionUpdateImportProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/permissions/update-import", {
        method: "POST",
        body: formData,
      });
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      link.href = url;
      link.download = "permissions_update_template.xlsx";
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold">Modification de permissions</h3>
          <p className="text-sm text-muted-foreground">
            Modifiez des permissions existantes en masse depuis un fichier Excel
          </p>
        </div>
        <Button
          variant="outline"
      ...
      <Alert>
        ...
        <AlertDescription>
          ...
          <ul className="list-disc list-inside mt-2 space-y-1 text-sm">
            ...
            <li>
              Le nom est obligatoire pour identifier la permission à modifier
            </li>
