...
import {
  validateTypeorganeImportData,
  TypeorganeImportResult,
  ImportError,
  ImportSummary,
} from "@/lib/validation/typeorgane-import.schema";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Valider les données
    const { valid, errors } = await validateTypeorganeImportData(
      mappedData,
      entrepriseId
    );
    ...
    for (const item of valid) {
      try {
        // Vérifier si le type d'organe existe déjà
        ...
        if (existing) {
          importErrors.push({
            ...
            field: "name",
            value: item.name,
            message: `Le type d'organe "${item.name}" existe déjà`,
            severity: "error",
          });
          summary.errors++;
        } else {
          // Créer le nouveau type d'organe
          const created = await prisma.typeOrgane.create({
            data: {
              name: item.name,
              entrepriseId,
            },
    ...
    const result: TypeorganeImportResult = {
      success: summary.errors === 0,
      message:
        summary.errors === 0
          ? `${summary.created} types d'organes créés avec succès`
    ...
    console.error("Erreur lors de l'importation des types d'organes:", error);
    return NextResponse.json(
      {
  ...
  try {
    ...
    for (let col = range.s.c; col <= range.e.c; col++) {
      ...
      if (header.includes("Nom*")) {
        comment = "Obligatoire. Nom unique du type d'organe.";
      }
    ...
    return new NextResponse(buffer, {
      headers: {
        "Content-Type":
          "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        "Content-Disposition": "attachment; filename=typeorganes_template.xlsx",
      },
    });
  } catch (error) {
    console.error("Erreur GET /api/typeorganes/import:", error);
    return NextResponse.json(
