...
const roleImportSchema = yup.object().shape({
...
export async function GET(request: NextRequest) {
  try {
    ...
    const templateData = [
      {
        "name*": "Administrateur",
        description: "Rôle avec tous les accès",
      },
      {
        "name*": "Utilisateur",
    ...
    for (let col = range.s.c; col <= range.e.c; col++) {
      ...
      if (header.includes("name")) {
        comment = "Obligatoire. Nom unique du rôle.";
      } else if (header.includes("description")) {
        comment = "Optionnel. Description du rôle.";
      }
    ...
    return new NextResponse(buffer, {
      headers: {
        ...
        "Content-Disposition": 'attachment; filename="roles_template.xlsx"',
      },
    });
  } catch (error) {
    console.error("Erreur GET /api/roles/import:", error);
...
// POST - Importer les rôles depuis Excel
export async function POST(request: NextRequest) {
  try {
    ...
    for (let i = 0; i < mappedData.length; i++) {
      ...
      try {
        // Valider les données
        await roleImportSchema.validate(rowData, { abortEarly: false });
    ...
    console.error("Erreur POST /api/roles/import:", error);
    return NextResponse.json(
      {
        success: false,
        message: "Erreur lors de l'importation des rôles",
