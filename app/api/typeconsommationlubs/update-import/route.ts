...
import {
  validateTypeconsommationlubUpdateImportData,
  TypeconsommationlubUpdateImportResult,
  ImportError,
  ImportSummary,
} from "@/lib/validation/typeconsommationlub-update-import.schema";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Valider les données
    const { valid, errors } = await validateTypeconsommationlubUpdateImportData(
      mappedData,
      entrepriseId
    );
    ...
    for (const item of valid) {
      try {
        // Trouver le type de consommation de lubrifiant existant
        ...
        if (!existing) {
          importErrors.push({
            ...
            field: "name",
            value: item.name,
            message: `Le type de consommation de lubrifiant "${item.name}" n'existe pas`,
            severity: "error",
          });
          summary.errors++;
        } else {
          // Pour ce cas simple, nous considérons que l'existence même du type dans le fichier
          // est une "modification" réussie (validation)
    ...
    const result: TypeconsommationlubUpdateImportResult = {
      success: summary.errors === 0,
      message:
        summary.errors === 0
          ? `${summary.updated} types de consommation de lubrifiants mis à jour avec succès`
    ...
    console.error(
      "Erreur lors de la mise à jour des types de consommation de lubrifiants:",
      error
    );
    return NextResponse.json(
      {
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Récupérer les types de consommation de lubrifiants existants
    const existingTypes = await prisma.typeconsommationlub.findMany({
      where: { entrepriseId },
      select: { name: true },
      take: 5,
    });
    ...
    const templateData = existingTypes.map((type) => ({
    ...
    for (let col = range.s.c; col <= range.e.c; col++) {
      ...
      if (header.includes("Nom du type")) {
        comment =
          "Obligatoire. Type de consommation de lubrifiant existant à modifier. Disponibles: " +
    ...
    return new NextResponse(buffer, {
      headers: {
        "Content-Type":
          "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        "Content-Disposition":
          "attachment; filename=typeconsommationlubs_update_template.xlsx",
      },
    });
  } catch (error) {
    console.error("Erreur GET /api/typeconsommationlubs/update-import:", error);
    return NextResponse.json(
