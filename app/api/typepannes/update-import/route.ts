...
import {
  validateTypepanneUpdateImportData,
  TypepanneUpdateImportResult,
  ImportError,
  ImportSummary,
} from "@/lib/validation/typepanne-update-import.schema";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Valider les données
    const { valid, errors } = await validateTypepanneUpdateImportData(
      mappedData,
      entrepriseId
    );
    ...
    for (const item of valid) {
      try {
        // Trouver le type de panne existant
        const existing = await prisma.typepanne.findFirst({
          where: {
            name: item.name,
            entrepriseId,
          },
        ...
        if (!existing) {
          importErrors.push({
            ...
            field: "name",
            value: item.name,
            message: `Le type de panne "${item.name}" n'existe pas`,
            severity: "error",
          });
          summary.errors++;
        } else {
          ...
          if (Object.keys(updateData).length > 0) {
            const updated = await prisma.typepanne.update({
    ...
    const result: TypepanneUpdateImportResult = {
      success: summary.errors === 0,
      message:
        summary.errors === 0
          ? `${summary.updated} types de pannes mis à jour avec succès`
    ...
    console.error("Erreur lors de la mise à jour des types de pannes:", error);
    return NextResponse.json(
      {
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Récupérer les types de pannes existants
    const existingTypepannes = await prisma.typepanne.findMany({
      where: { entrepriseId },
    ...
    const templateData = existingTypepannes.map((typepanne, index) => ({
      "Nom*": typepanne.name,
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Types de pannes");
    ...
    for (let col = range.s.c; col <= range.e.c; col++) {
      ...
      if (header.includes("Nom*")) {
        comment =
          "Obligatoire. Type de panne existant à modifier. Disponibles: " +
          existingTypepannes.map((t) => t.name).join(", ");
    ...
    return new NextResponse(buffer, {
      headers: {
        "Content-Type":
          "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        "Content-Disposition":
          "attachment; filename=typepannes_update_template.xlsx",
      },
    });
  } catch (error) {
    console.error("Erreur GET /api/typepannes/update-import:", error);
    return NextResponse.json(
