...
import {
  validateTypepanneImportData,
  TypepanneImportResult,
  ImportError,
  ImportSummary,
} from "@/lib/validation/typepanne-import.schema";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Valider les données
    const { valid, errors } = await validateTypepanneImportData(
      mappedData,
      entrepriseId
    );
      ...
      try {
        ...
        if (existing) {
          importErrors.push({
            ...
            field: "name",
            value: item.name,
            message: `Le type de panne "${item.name}" existe déjà`,
            severity: "error",
          });
          summary.errors++;
        } else {
          // Créer le nouveau type de panne
          const created = await prisma.typepanne.create({
            data: {
              name: item.name,
              entrepriseId,
            },
    ...
    const result: TypepanneImportResult = {
      success: summary.errors === 0,
      message:
        summary.errors === 0
          ? `${summary.created} types de pannes créés avec succès`
    ...
    console.error("Erreur lors de l'importation des types de pannes:", error);
    return NextResponse.json(
      {
  ...
  try {
    ...
    for (let col = range.s.c; col <= range.e.c; col++) {
      ...
      if (header.includes("Nom*")) {
        comment = "Obligatoire. Nom unique du type de panne.";
      }
    ...
    return new NextResponse(buffer, {
      headers: {
        "Content-Type":
          "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        "Content-Disposition": "attachment; filename=typepannes_template.xlsx",
      },
    });
  } catch (error) {
    console.error("Erreur GET /api/typepannes/import:", error);
    return NextResponse.json(
