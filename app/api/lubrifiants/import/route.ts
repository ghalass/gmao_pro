...
import {
  validateLubrifiantImportData,
  LubrifiantImportResult,
  ImportError,
  ImportSummary,
} from "@/lib/validation/lubrifiant-import.schema";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Valider les données
    const { valid, errors } = await validateLubrifiantImportData(
      mappedData,
      entrepriseId
    );
    ...
    for (const item of valid) {
      try {
        // Mapper le nom du type de lubrifiant vers l'ID
        const typelubrifiantId = item.typelubrifiantName
          ? typeLubrifiantMap.get(item.typelubrifiantName.toLowerCase())
          : null;
        ...
        // Vérifier si le lubrifiant existe déjà
        ...
        if (existing) {
          importErrors.push({
            ...
            field: "name",
            value: item.name,
            message: `Le lubrifiant "${item.name}" existe déjà`,
            severity: "error",
          });
          summary.errors++;
        } else {
          // Créer le nouveau lubrifiant
          const created = await prisma.lubrifiant.create({
            data: {
              name: item.name,
              typelubrifiantId,
              entrepriseId,
    ...
    const result: LubrifiantImportResult = {
      success: summary.errors === 0,
      message:
        summary.errors === 0
          ? `${summary.created} lubrifiants créés avec succès`
    ...
    console.error("Erreur lors de l'importation des lubrifiants:", error);
    return NextResponse.json(
      {
  ...
  try {
    ...
    for (let col = range.s.c; col <= range.e.c; col++) {
      ...
      if (header.includes("Nom*")) {
        comment = "Obligatoire. Nom unique du lubrifiant.";
      } else if (header.includes("Type lubrifiant*")) {
        comment =
          "Obligatoire. Type de lubrifiant associé. Disponibles: " +
    ...
    return new NextResponse(buffer, {
      headers: {
        "Content-Type":
          "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        "Content-Disposition": "attachment; filename=lubrifiants_template.xlsx",
      },
    });
  } catch (error) {
    console.error("Erreur GET /api/lubrifiants/import:", error);
    return NextResponse.json(
