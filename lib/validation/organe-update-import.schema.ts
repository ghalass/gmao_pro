...
export interface OrganeUpdateImportData {
  name: string; // Used as identifier
  typeOrganeName: string; // Used as identifier
  newName?: string; // Optional new name
  newTypeOrganeName?: string; // Optional new type
  marque?: string;
  sn?: string;
  date_mes?: Date;
...
export interface OrganeUpdateImportResult {
  success: boolean;
  message: string;
  data?: OrganeUpdateImportData[];
  errors?: ImportError[];
...
export const organeUpdateImportSchema: yup.ObjectSchema<OrganeUpdateImportData> =
  yup.object({
    name: yup
      .string()
      .required("Le nom de l'organe à modifier est obligatoire")
      .min(1, "Le nom ne peut pas être vide"),

    typeOrganeName: yup
      .string()
      .required("Le type d'organe est obligatoire pour l'identification")
    ...
    newTypeOrganeName: yup
      .string()
      .optional()
      .min(1, "Le nouveau type d'organe ne peut pas être vide"),
...
export const validateOrganeUpdateImportData = async (
  data: any[],
  entrepriseId: string
): Promise<{ valid: OrganeUpdateImportData[]; errors: ImportError[] }> => {
  const errors: ImportError[] = [];
  const valid: OrganeUpdateImportData[] = [];
  ...
  for (let index = 0; index < data.length; index++) {
    ...
    try {
      ...
      const validatedData = await organeUpdateImportSchema.validate(row, {
        abortEarly: false,
        stripUnknown: true,
      });
...
export const generateUpdateExcelTemplate = (
  ...
  const template = existingData.map((item, index) => ({
    "Nom*": item.name,
    "Type organe*": item.type_organe?.name || "",
    "Nouveau nom": index === 0 ? "Nouveau nom exemple" : "",
    "Nouveau type organe":
      index === 0 ? (typeOrganes.length > 1 ? typeOrganes[1].name : "") : "",
    Marque: item.marque || "",
    "Numéro de série": item.sn || "",
    "Date de mise en service": item.date_mes
      ? new Date(item.date_mes).toLocaleDateString("fr-FR")
      : "",
    Origine: item.origine || "",
    Circuit: item.circuit || "",
    "HRM initial": item.hrm_initial || "",
