...
export interface TypeparcImportData {
  name: string;
}

export interface TypeparcImportResult {
  success: boolean;
  message: string;
  data?: TypeparcImportData[];
  errors?: ImportError[];
...
export const typeparcImportSchema: yup.ObjectSchema<TypeparcImportData> =
  yup.object({
    name: yup
      .string()
...
export const typeparcUpdateImportSchema: yup.ObjectSchema<TypeparcImportData> =
  yup.object({
    name: yup
      .string()
      .required("Le nom du type de parc est obligatoire")
...
// Fonction de validation pour l'importation
export const validateTypeparcImportData = async (
  data: any[]
): Promise<TypeparcImportResult> => {
  const errors: ImportError[] = [];
  const validData: TypeparcImportData[] = [];
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    try {
      const validatedRow = await typeparcImportSchema.validate(row, {
        abortEarly: false,
      });
