...
export interface TypeorganeUpdateImportData {
  name: string; // Used as identifier
  newName?: string; // Optional new name
}

export interface TypeorganeUpdateImportResult {
  success: boolean;
  message: string;
  data?: TypeorganeUpdateImportData[];
  errors?: ImportError[];
...
export const typeorganeUpdateImportSchema: yup.ObjectSchema<TypeorganeUpdateImportData> =
  yup.object({
    name: yup
      .string()
      .required("Le nom du type d'organe à modifier est obligatoire")
...
export const validateTypeorganeUpdateImportData = async (
  data: any[],
  entrepriseId: string
): Promise<{ valid: TypeorganeUpdateImportData[]; errors: ImportError[] }> => {
  const errors: ImportError[] = [];
  const valid: TypeorganeUpdateImportData[] = [];
  ...
  for (let index = 0; index < data.length; index++) {
    ...
    try {
      ...
      const validatedData = await typeorganeUpdateImportSchema.validate(row, {
        abortEarly: false,
        stripUnknown: true,
      });
