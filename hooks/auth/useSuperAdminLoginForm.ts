// hooks/useSuperAdminLoginForm.ts
...
export function useSuperAdminLoginForm() {
  ...
  const loginWithAllData = React.useCallback(
    async (email: string, password: string) => {
      return apiFetch(API.AUTH.SUPER_ADMIN_LOGIN, {
        method: methods.POST,
        body: {
          email,
          password,
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      try {
        ...
        const response = await loginWithAllData(value.email, value.password);

        if (response.ok) {
          await refreshUser();
          toast.success("Connecté avec succès");
          router.push(ROUTE.SUPER_ADMIN.DASHBOARD);
        } else {
